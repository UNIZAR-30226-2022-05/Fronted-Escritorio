...
public class AmigosController implements Initializable{
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		apirest = App.apiweb.getRestAPI();
		apirest.setOnError(e -> {
			if(DEBUG) System.out.println(e);
			labelError.setText(StringUtils.parseString(e.toString()));
		});
		
		apirest.openConnection("/api/sacarPeticionesEnviadas");
    	apirest.receiveObject(ListaUsuarios.class, enviadas -> {
    ...
    private void findFriend(ActionEvent event) {
		...
		if (correoAmigo.equals("")) {
			...
			apirest.openConnection("/api/buscarAmigo");
			apirest.receiveObject(ListaUsuarios.class, usuarios -> {
				//COMPROBAR SI HA HABIDO ALGÚN ERROR
				String error = usuarios.getError();
				if (error.equals("null")) {
